...
def header(page, root):
    routes = [('initiatives','Work','pages/initiatives.html'),('profile','About','pages/profile.html'),('notes','Field Notes','pages/field-notes.html'),('insights','Evidence','pages/insights.html')]
    ...
    return f'''<div class="site-header-wrap site-shell" id="site-header"><header class="site-header">
      <a class="brand" href="{root}index.html" aria-label="S/S · Signal &amp; Self · Mina Soliman’s fieldbook · Home"><span class="brand-mark" aria-hidden="true">S/S</span><span class="brand-copy"><strong>Signal &amp; Self</strong><small>Mina Soliman’s fieldbook</small></span></a>
...
def page(path, key, title, description, content, module=None, schema=None):
    ...
    person={'@type':'Person','@id':BASE+'#mina','name':'Mina Soliman','url':BASE,'sameAs':[D['profile']['github']],'description':D['profile']['tagline']}
...
<link rel="preload" href="https://fonts.gstatic.com/s/newsreader/v26/cY9AfjOCX1hbuyalUrK4397yjIJFJpc.woff2" as="font" type="font/woff2" crossorigin>
...
<link rel="stylesheet" href="{root}assets/styles/portfolio.css?v={VERSION}">
...
def home():
    intro=f'''<section class="home-hero site-shell"><div class="hero-copy"><p class="eyebrow">Electrical engineering · responsible technology</p><h1>Building systems that deserve <span class="serif-italic">trust.</span></h1><p class="hero-lede">{e(D['profile']['hero'])}</p><div class="button-row"><a class="button button-primary" href="#featured-work">View engineering projects</a><a class="button button-quiet" href="#fieldbook">Explore the fieldbook</a></div><div class="professional-links">{link('GitHub',D['profile']['github'])} {link('About Mina','pages/profile.html')}<span>Updated {date.fromisoformat(D['site']['lastUpdated']).strftime('%B %Y')}</span></div></div><aside class="hero-index" aria-label="Engineering areas"><span class="mono-label">Signal &amp; Self / 01</span><p>Power.<br>Systems.<br><em>Responsibility.</em></p><span class="mono-label">University of Toronto<br>First year · 2026–27</span></aside></section>'''
    ...
    compass='''<div class="compass-layout"><div><p class="eyebrow">Why Signal &amp; Self exists</p><h2>Skill matters.<br><span class="serif-italic">So does judgment.</span></h2><p>Engineering work raises human questions: what can I verify, who should I protect, what can I build, and what should change after reflection?</p><p>This fieldbook keeps those questions beside the code.</p><a class="button button-quiet" href="pages/profile.html#framework">Explore the framework</a></div><div class="hero-visual" aria-hidden="true"><div class="orbit-stage"><div class="orbit-core"><strong>Skill + care = trust</strong></div><span class="orbit-label">Verify</span><span class="orbit-label">Protect</span><span class="orbit-label">Build</span><span class="orbit-label">Reflect</span></div></div></div>'''
    intro+=f'<section class="section site-shell" id="fieldbook">{compass}</section>'
    ...
    page('index.html','home','Mina Soliman — Electrical Engineering & Responsible Technology Portfolio',f'Signal & Self: Mina Soliman’s University of Toronto Electrical Engineering portfolio, with {len(FEATURED)} tested repositories and a responsible-technology fieldbook.',intro)
...
def privacy():
    ...
    content+=section('External requests','Know when another service is contacted.','<div class="panel"><p>GitHub Pages serves these public files. Google Fonts supplies the typefaces. Cloudflare’s beacon is loaded only on mini34.github.io, never in local previews. Blocking a font, analytics, or identity request does not gate the portfolio.</p><p>Opening the account panel contacts Google to load its sign-in interface. Choosing an account is a further action. Google receives the requests needed for that interface; session personalization here does not provide authentication for private content.</p><p>A profile image may be requested from Google after sign-in. The image request uses a no-referrer policy. External links navigate in the same tab unless you choose otherwise.</p><p>Cloudflare analytics use their standard aggregate beacon configuration. No visitor identity, local name, goal, saved-item list, or audience is sent as custom analytics data.</p></div>')
